class ModelReader {
  ...
  static _parse(modelString) {
    const regex = {
      ...
      modelparts: new RegExp(
        /\s*(\/\/(.*?)\r?\n)/.source +
          "|" + // Comments
          /\s*(texture|light|model|material|voxels)\s+/.source +
          "|" + // SVOX Blocks
          /\s*([^=,\r\n]+=\s*data:image.*?base64,.*$)\s*/.source +
          "|" + // Name = data:image/...;base64,iVBORw...
          /\s*([^=,\r\n]+=[^\r\n=;,\/]+)\s*/.source +
          "|" + // Name = Value
    ...
    let lines = Array.from(
      modelString
        .replaceAll(regex.linecontinuation, " ")
        .matchAll(regex.modelparts),
      (m) => m[0].trim()
    );
  ...
  static _createModel(modelData) {
    ...
    model.rotation = this._parseXYZFloat(
      "rotation",
      modelData.rotation,
      "0 0 0",
      false
    );
    model.position = this._parseXYZFloat(
      "position",
      modelData.position,
      "0 0 0",
      false
    ...
    if (modelData.resize === SVOX.BOUNDS) model.resize = SVOX.BOUNDS;
    else if (modelData.resize === SVOX.MODEL) model.resize = SVOX.MODEL;
    else if (modelData.resize) model.resize = null;
    ...
    if (modelData.lights.some((light) => light.size)) {
      ...
      let lightMaterial = model.materials.createMaterial(
        SVOX.MATBASIC,
        SVOX.FLAT,
        1,
        0,
  ...
  static _createLight(model, lightData) {
    ...
    lightData.direction = this._parseXYZFloat(
      "direction",
      lightData.direction,
      null,
      false
    );
    lightData.position = this._parseXYZFloat(
      "position",
      lightData.position,
      null,
      false
    ...
    lightData.detail = Math.min(
      3,
      Math.max(0, parseInt(lightData.detail || 1, 10))
    );
    let light = new Light(
      lightData.color,
      lightData.strength,
  ...
  static _createMaterial(model, materialData) {
    ...
    if (!materialData.emissive) {
      if (materialData.emissivemap) materialData.emissive = "#FFF 1";
      else materialData.emissive = "#000 0";
    }
    ...
    let material = model.materials.createMaterial(
      ...
      lighting,
      parseFloat(
        materialData.roughness || (materialData.roughnessmap ? 1.0 : 1.0)
      ),
      parseFloat(
        materialData.metalness || (materialData.metalnessmap ? 1.0 : 0.0)
      ),
      ...
      materialData.roughnessmap
        ? model.textures[materialData.roughnessmap]
        : null,
      materialData.metalnessmap
        ? model.textures[materialData.metalnessmap]
        : null,
      materialData.emissivemap
        ? model.textures[materialData.emissivemap]
        : null,
      materialData.matcap ? model.textures[materialData.matcap] : null,
      materialData.reflectionmap
        ? model.textures[materialData.reflectionmap]
        : null,
      materialData.refractionmap
        ? model.textures[materialData.refractionmap]
        : null,
    ...
    if (materialData.deform) {
      ...
      material.setDeform(
        parseFloat(materialData.deform.split(" ")[0]), // Count
        parseFloat(materialData.deform.split(" ")[1] || 1.0), // Strength
        parseFloat(materialData.deform.split(" ")[2] || 1.0)
      ); // Damping
    }
    ...
    const CLEANDEFINITION =
      /([A-Z][a-z]*)\s*(\(\d+\))?[:]\s*(#[a-fA-F0-9]*)\s*/g;
    materialData.colors = materialData.colors.replace(CLEANCOLORID, "($1)");
    materialData.colors = materialData.colors.replace(
      CLEANDEFINITION,
      "$1$2:$3 "
    );
  ...
  static _createVoxels(model, voxels) {
    ...
    let voxelLength = 0;
    const voxChunk = (model.voxChunk = new VoxChunk([
      model.size.x,
      model.size.y,
      model.size.z,
    ]));
    ...
    for (let i = 0; i < rleArray.length; i++) {
      ...
      if (rleArray[i][0] !== "-") {
        ...
        materialIndex = model.materials.materials.findIndex((m) =>
        ...
        if (!color) {
          ...
          if (errorMaterial === null)
            errorMaterial = model.materials.createMaterial(
              SVOX.MATSTANDARD,
              SVOX.FLAT,
              0.5,
              0.0,
      ...
      if (shellData !== "none") {
        ...
        if (parts.length < 2 || parts.length % 2 !== 0) {
          ...
          for (let s = 0; s < parts.length / 2; s++) {
            ...
            if (
              !/^[A-Z][a-z]*$/.test(colorId) ||
              !/^([-+]?[0-9]*\.?[0-9]+)*$/.test(distance)
            ) {
              error = true;
              break;
            } else
  ...
  static _parseVertexData(vertexData, modelType) {
    if (vertexData) {
      ...
      for (let i = 0; i < modelData.length; i++) {
        error =
          error ||
          modelData[i].values.length === 0 ||
          modelData[i].values.length >= 4;
      }
      if (error) {
        throw {
          name: "SyntaxError",
    ...
    try {
      if ((modelData || materialData) && materialData) {
        ...
        for (let i = 0; i < modelData.length; i++) {
          ...
          error =
            error ||
            modelData[i].values.length !== materialData[i].values.length;
        }
      }
    } catch (ex) {
      error = true;
  ...
  static _validateProperties(data, mandatory, optional, objectName) {
    ...
    for (let propertyName in data) {
      if (
        !mandatory.includes(propertyName) &&
        !optional.includes(propertyName)
      ) {
        throw {
          name: "SyntaxError",
