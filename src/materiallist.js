...
class Color {
  ...
  _setMaterial(material) {
    if (this._material !== undefined) throw "A Color can only be added once.";
...
class BaseMaterial {
  constructor(
    ...
    if (
      ((map && map.cube) ||
        ...
        (roughnessMap && roughnessMap.cube) ||
        (metalnessMap && metalnessMap.cube) ||
        (emissiveMap && emissiveMap.cube)) &&
      !(uscale === -1 && vscale === -1)
    ) {
      throw {
        name: "SyntaxError",
    ...
    this.refractionRatio =
      typeof refractionRatio === "number" ? refractionRatio : 0.9;
  ...
  setEmissive(color, intensity) {
    ...
    else this._emissive = { color: Color.fromHex(color), intensity: intensity };
  }
...
class Material {
  ...
  setDeform(count, strength, damping) {
    ...
    else this._deform = { count: 0, strength: 0, damping: 0 };
  }
...
class MaterialList {
  ...
  createMaterial(
    ...
    if (![SVOX.FRONT, SVOX.BACK, SVOX.DOUBLE].includes(side)) side = SVOX.FRONT;
