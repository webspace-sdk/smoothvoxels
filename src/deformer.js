class Deformer {
  static changeShape(model, buffers, shape) {
    switch (shape) {
      ...
      default:
        ...
        for (
          let faceIndex = 0, c = model.faceCount;
          faceIndex < c;
          faceIndex++
        ) {
  ...
  static _circularDeform(model, buffers, xStrength, yStrength, zStrength) {
    ...
    for (let vertIndex = 0, c = model.vertCount; vertIndex < c; vertIndex++) {
      ...
      const vertexDistance = Math.sqrt(
        x * x * xStrength + y * y * yStrength + z * z * zStrength
      );
  ...
  static _markEquidistantFaces(model, buffers) {
    ...
    for (let faceIndex = 0, c = model.faceCount; faceIndex < c; faceIndex++) {
      ...
      faceEquidistant.set(
        faceIndex,
        vertRing[faceVertIndices[faceVertIndex0]] ===
          vertRing[faceVertIndices[faceVertIndex1]] &&
          vertRing[faceVertIndices[faceVertIndex0]] ===
            vertRing[faceVertIndices[faceVertIndex2]] &&
          vertRing[faceVertIndices[faceVertIndex0]] ===
            vertRing[faceVertIndices[faceVertIndex3]]
          ? 1
          : 0
      );
    }
  ...
  static deform(model, buffers, maximumDeformCount) {
    ...
    for (let step = 0; step < maximumDeformCount; step++) {
      ...
      for (let vertIndex = 0, c = model.vertCount; vertIndex < c; vertIndex++) {
        ...
        const notClampOrFlattenX =
          1 - (vertClampedX.get(vertIndex) | vertFlattenedX.get(vertIndex));
        const notClampOrFlattenY =
          1 - (vertClampedY.get(vertIndex) | vertFlattenedY.get(vertIndex));
        const notClampOrFlattenZ =
          1 - (vertClampedZ.get(vertIndex) | vertFlattenedZ.get(vertIndex));
        ...
        vertHasTmp.set(
          vertIndex,
          notClampOrFlattenX | notClampOrFlattenY | notClampOrFlattenZ
        );
      }

      if (hasDeforms) {
        for (
          let vertIndex = 0, c = model.vertCount;
          vertIndex < c;
          vertIndex++
        ) {
  ...
  static warpAndScatter(model, buffers) {
    ...
    let [vxMinX, vxMaxX, vxMinY, vxMaxY, vxMinZ, vxMaxZ] = xyzRangeForSize(
      model.voxChunk.size
    );
    ...
    for (let vertIndex = 0, c = model.vertCount; vertIndex < c; vertIndex++) {
      ...
      if (hasAmplitude || hasScatter) {
        ...
        if (hasAmplitude) {
          xOffset =
            noise((vx + 0.19) * frequency, vy * frequency, vz * frequency) *
            amplitude;
          yOffset =
            noise((vy + 0.17) * frequency, vz * frequency, vx * frequency) *
            amplitude;
          zOffset =
            noise((vz + 0.13) * frequency, vx * frequency, vy * frequency) *
            amplitude;
        }
