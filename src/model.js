...
class Model {
  ...
  set clamp(clamp) {
    this._clamp = Planar.parse(clamp);
  }
  get clamp() {
  ...
  set shape(shape) {
    ...
    if (
      !["box", "sphere", "cylinder-x", "cylinder-y", "cylinder-z"].includes(
        this._shape
      )
    ) {
      throw {
  ...
  prepareForRender(buffers) {
    const {
      tmpVertIndexLookup,
      tmpVoxelXZYFaceIndices,
      tmpVoxelXYZFaceIndices,
      tmpVoxelYZXFaceIndices,
    } = buffers;
    const { voxChunk } = this;
    ...
    for (let vx = minX; vx <= maxX; vx++) {
      for (let vy = minY; vy <= maxY; vy++) {
        for (let vz = minZ; vz <= maxZ; vz++) {
          ...
          for (
            let faceNameIndex = 0, l = SVOX._FACES.length;
            faceNameIndex < l;
            faceNameIndex++
          ) {
            ...
            const created = this._createFace(
              voxChunk,
              buffers,
              materials,
              vx,
    ...
    buffers.voxelXZYFaceIndices = tmpVoxelXZYFaceIndices.slice(
      0,
      this.faceCount
    );
    buffers.voxelXYZFaceIndices = tmpVoxelXYZFaceIndices.slice(
      0,
      this.faceCount
    );
    buffers.voxelYZXFaceIndices = tmpVoxelYZXFaceIndices.slice(
      0,
      this.faceCount
    );
  ...
  _createFace(
    ...
    if (material.opacity === 0) {
      ...
      const neightborMaterialIndex =
        (voxChunk.colorForPaletteIndex(neighborPaletteIndex) & 0xff000000) >>
        24;
    ...
    let flattened = this._isFacePlanar(
      material,
      vx,
      vy,
      vz,
    ...
    let clamped = this._isFacePlanar(
      material,
      vx,
      vy,
      vz,
    ...
    let skipped = this._isFacePlanar(
      material,
      vx,
      vy,
      vz,
    ...
    faceVertIndices[faceVertOffset] = this._createVertex(
      buffers,
      material,
      vx,
      vy,
    ...
    faceVertIndices[faceVertOffset + 1] = this._createVertex(
      buffers,
      material,
      vx,
      vy,
    ...
    faceVertIndices[faceVertOffset + 2] = this._createVertex(
      buffers,
      material,
      vx,
      vy,
    ...
    faceVertIndices[faceVertOffset + 3] = this._createVertex(
      buffers,
      material,
      vx,
      vy,
  ...
  _createVertex(
    ...
    if (vertIndexLookup.has(key)) {
      ...
      if (!deform) {
        ...
        vertDeformCount[vertIndex] !== 0 &&
        this._getDeformIntegral(material.deform) <
          this._getDeformIntegralAtVertex(buffers, vertIndex)
      ) {
      ...
      if (!warp) {
        ...
        (warp.amplitude < vertWarpAmplitude[vertIndex] ||
          (warp.amplitude === vertWarpAmplitude[vertIndex] &&
            warp.frequency > vertWarpFrequency[vertIndex]))
      ) {
      ...
      if (!scatter) vertScatter[vertIndex] = 0;
      else if (
  ...
  _getDeformIntegral(deform) {
    ...
    return deform.damping === 1
      ...
      : (deform.strength * (1 - Math.pow(deform.damping, deform.count + 1))) /
          (1 - deform.damping);
  }
  ...
  _isFacePlanar(
    ...
    switch (faceNameIndex) {
      case 0:
        return planar.x || (planar.nx && vx === bounds.minX); // nx
      case 1:
        return planar.x || (planar.px && vx === bounds.maxX); // px
      case 2:
        return planar.y || (planar.ny && vy === bounds.minY); // ny
      case 3:
        return planar.y || (planar.py && vy === bounds.maxY); // py
      case 4:
        return planar.z || (planar.nz && vz === bounds.minZ); // nz
      case 5:
        return planar.z || (planar.pz && vz === bounds.maxZ); // pz
      default:
        return false;
    }
  }
  ...
  _setIsVertexPlanar(
    ...
    if (planar) {
      ...
      arrX.set(
        vertIndex,
        planar.x ||
          (planar.nx && vx < bounds.minX + 0.5) ||
          (planar.px && vx > bounds.maxX + 0.5)
          ? 1
          : arrX.get(vertIndex) | 0
      );
      arrY.set(
        vertIndex,
        planar.y ||
          (planar.ny && vy < bounds.minY + 0.5) ||
          (planar.py && vy > bounds.maxY + 0.5)
          ? 1
          : arrY.get(vertIndex) | 0
      );
      arrZ.set(
        vertIndex,
        planar.z ||
          (planar.nz && vz < bounds.minZ + 0.5) ||
          (planar.pz && vz > bounds.maxZ + 0.5)
          ? 1
          : arrZ.get(vertIndex) | 0
      );
    } else {
