class ModelWriter {
  ...
  static writeToString(model, compressed, repeat) {
    ...
    let voxelWidth =
      compressed || maxIdLength === 1 || maxIdLength > 3 ? 1 : maxIdLength;
    ...
    if (size.y === size.x && size.z === size.x)
      ...
      result += `size = ${size.x * repeat} ${size.y * repeat} ${
        size.z * repeat
      }\r\n`;
    ...
    if (
      ...
      if (model.scale.y === model.scale.x && model.scale.z === model.scale.x)
        ...
        result += `scale = ${model.scale.x / repeat} ${
          model.scale.y / repeat
        } ${model.scale.z / repeat}\r\n`;
    }
    ...
    if (model.ao)
      result += `ao =${
        model.ao.color.toString() !== "#000" ? " " + model.ao.color : ""
      } ${model.ao.maxDistance} ${model.ao.strength}${
        model.ao.angle !== 70 ? " " + model.ao.angle : ""
      }\r\n`;
    ...
    if (model.data)
      result += `data = ${this._serializeVertexData(model.data)}\r\n`;
    ...
    else result += this._serializeVoxelsRLE(model, 100);
  ...
  static _serializeMaterials(model) {
    ...
    model.materials.forEach(function (material) {
      ...
      if (material.type !== SVOX.MATSTANDARD)
        settings.push(`type = ${material.type}`);
      if (material.lighting !== SVOX.fLAT)
      ...
      if (material.emissive)
        settings.push(
          `emissive = ${material.emissive.color} ${material.emissive.intensity}`
        );
      if (!material.fog) settings.push(`fog = false`);
      if (material.side !== SVOX.FRONT)
        settings.push(`side = ${material.side}`);

      if (material.deform)
        settings.push(
          `deform = ${material.deform.count} ${material.deform.strength}${
            material.deform.damping !== 1 ? " " + material.deform.damping : ""
          }`
        );
      if (material.warp)
        settings.push(
          `warp = ${material.warp.amplitude} ${material.warp.frequency}`
        );
      if (material.scatter) settings.push(`scatter = ${material.scatter}`);

      if (material.ao)
        settings.push(
          `ao =${material.ao.color !== "#000" ? " " + material.ao.color : ""} ${
            material.ao.maxDistance
          } ${material.ao.strength}${
            material.ao.angle !== 70 ? " " + material.ao.angle : ""
          }`
        );
      ...
      if (
        material.mapTransform.uscale !== -1 ||
        material.mapTransform.vscale !== -1
      ) {
        let transform = `maptransform =`;
        transform += ` ${material.mapTransform.uscale} ${material.mapTransform.vscale}`;
        if (
          material.mapTransform.uoffset !== 0 ||
          material.mapTransform.voffset !== 0 ||
          material.mapTransform.rotation !== 0
        ) {
  ...
  static _serializeVoxels(model, repeat, voxelWidth) {
    let emptyVoxel = "-" + " ".repeat(Math.max(voxelWidth - 1));
  ...
  static _addRleChunk(queue, color, count, compressionWindow) {
    ...
    for (
      let k = Math.max(0, queue.length - compressionWindow * 2);
      k <= queue.length - 2;
      k++
    ) {
      let item = queue[k][0];
