...
class AOCalculator {
  static calculateAmbientOcclusion(model, buffers) {
    ...
    for (let faceIndex = 0; faceIndex < faceCount; faceIndex++) {
      ...
      let max =
        ao.maxDistance * Math.max(modelScaleX, modelScaleY, modelScaleZ);
      ...
      for (let v = 0; v < 4; v++) {
        ...
        for (const [directionX, directionY, directionZ] of samples) {
          ...
          let distance = this._distanceToOctree(
            model,
            buffers,
            octree,
            originX,
  ...
  static _trianglesToOctree(triangles, model, buffers) {
    ...
    if (length <= 32) {
      ...
      for (let index = 0; index < 8; index++) {
        ...
        const subPartition = this._trianglesToOctree(
          subTriangles[index],
          model,
          buffers
        );
  ...
  static _distanceToOctree(
    ...
    if (
      this._hitsBox(originX, originY, originZ, endX, endY, endZ, octree) ===
      false
    )
      return null;

    if (octree.triangles.length > 0) {
      return this._distanceToModel(
        model,
        buffers,
        octree.triangles,
        originX,
    ...
    for (let index = 0; index < 8; index++) {
      ...
      let dist = this._distanceToOctree(
        model,
        buffers,
        partition,
        originX,
  ...
  static _aoSidesToOctree(model, buffers, octree) {
    const bounds = model.determineBoundsOffsetAndRescale(
      SVOX.MODEL,
      buffers
    ).bounds;
    ...
    const pushNewTriangleIntoFaceVerts = (
      x0,
      y0,
      z0,
      x1,
  ...
  static _distanceToModel(
    ...
    for (let t = 0; t < triangles.length; t++) {
      ...
      let dist = this._triangleDistance(
        model,
        buffers,
        vert0Index,
        vert1Index,
  ...
  static _generateOctahedronSamples(verticalCount) {
    ...
    for (let vc = 0; vc <= verticalCount; vc++) {
      ...
      for (let hc = 0; hc < horizontalCount; hc++) {
        ...
        if (vc < verticalCount) samples.push({ x: x, y: -y, z: z });
      }
