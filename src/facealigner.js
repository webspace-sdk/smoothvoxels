class FaceAligner {
  ...
  static alignFaceDiagonals(model, buffers) {
    ...
    for (let faceIndex = 0, c = model.faceCount; faceIndex < c; faceIndex++) {
      ...
      const distance1toMid =
        (vert1X - mid02X) * (vert1X - mid02X) +
        (vert1Y - mid02Y) * (vert1Y - mid02Y) +
        (vert1Z - mid02Z) * (vert1Z - mid02Z);
      const distance3toMid =
        (vert3X - mid02X) * (vert3X - mid02X) +
      ...
      const distance0toMid =
        (vert0X - mid13X) * (vert0X - mid13X) +
        (vert0Y - mid13Y) * (vert0Y - mid13Y) +
        (vert0Z - mid13Z) * (vert0Z - mid13Z);
      const distance2toMid =
        (vert2X - mid13X) * (vert2X - mid13X) +
      ...
      if (distance1toMid < maxDist || distance3toMid < maxDist) {
        ...
        while (
          this._getVertexSumInline(vert1X, vert1Y, vert1Z) < min ||
